...
/***/ function(module, exports, __webpack_require__) {
	...
	//variables
	var xImg = '<img class="played" src="./assets/pokeX.gif">'; //change this if you change the img file for x.
	var oImg = '<img class="played" src="./assets/pokeO.gif">'; //change this if you change the img file for o.
	var pokemonX = 'Bulbasaur';
	var pokemonO = 'Charmander';
	var turn = 0; //x's are even, o's are odd
	var boardArray = ["", "", "", "", "", "", "", "", ""];
	var gameOver = false;
	...
	//display who wins
	var onWinner = function onWinner(char) {
	  if (char === 'x') {
	    char = pokemonX;
	  } else if (char === 'o') {
	    char = pokemonO;
	...
	var onNewGame = function onNewGame() {
	  ...
	  api.newGame(data).done(ui.createGameSuccess).fail(ui.failure);
	};

	//send moves to server
	var updateGame = function updateGame(i, v, o) {
	  var data = {
	    "game": {
	      "cell": {
	        "index": i,
	  ...
	  api.updateTheGame(data).done(ui.success).fail(ui.failure);
	};

	//make changes to screen from what we loaded form server
	var displayLoad = function displayLoad(data) {
	...
	//user loads games
	var onLoadGame = function onLoadGame() {
	  $('#load-game-modal').modal('show');
	  $('#load-game-submit').on('click', function () {
	    //get text fields
	    var gameId = $('#load-game-id').val();
	    api.loadGame(gameId).done(displayLoad).fail(ui.failure);
	...
	//user sees all games
	var onSeeAllGames = function onSeeAllGames() {
	  $('#show-all-games-modal').modal('show');
	  //let data;
	  api.seeAllGames().done(displayAllGames).fail(ui.failure);
	};

	//user clear board
	var onClearBoard = function onClearBoard() {
	  //clear array
	  boardArray = ["", "", "", "", "", "", "", "", ""];
	  $('.square').data('closed', 0);
	  //clear images
	...
	var checkWinner = function checkWinner(array, char, turns) {
	  //check colums
...
/***/ function(module, exports, __webpack_require__) {
	...
	var appVar = __webpack_require__(6);
	...
	var updateTheGame = function updateTheGame(data) {
	  return $.ajax({
	...
	var loadGame = function loadGame(data) {
	  return $.ajax({
	...
	var seeAllGames = function seeAllGames() {
	  return $.ajax({
	    url: appVar.app.api + 'games/',
	    method: 'GET',
	    headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function(global) {module.exports = global["getFormFields"] = __webpack_require__(20);
	/* WEBPACK VAR INJECTION */}.call(exports, (function() { return this; }())))
